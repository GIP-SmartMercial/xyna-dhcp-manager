...
function Gruntfile(grunt) {
    ...
    var config = {
        ...
        //Titel der Anwendung
        appTitle: "DHCP-Manager",

        //Name der WAR-Datei und des Pfades, in den später ein Release deployt wird
        appName: "dhcp-manager",

        //Language Argument
        langSuffix: ((grunt.option("lang") && !grunt.option("suppressLangSuffix")) ? "_" + grunt.option('lang') : (!grunt.option("lang") ? "_native" : "")),

        unobfuscated: grunt.option("unobfuscated") ? "-unobfuscated" : "",
    ...
    grunt.registerTask("release", [
        ...
        "release-modeler" + (grunt.option("lang") ? "::" + grunt.option('lang') : ""),
    ...
    grunt.registerTask("release-modeler", function(task, lang) {
        console.log("with args: " + task + ', ' + lang + ', ' + config.unobfuscated);
        var exec = require("child_process").exec;
        var cb = this.async();
        exec("npm run release" + config.unobfuscated + (lang ? ":" + lang : ""), {maxBuffer : 500 * 1024, cwd: "../impl"}, function(err, stdout, stderr) {
            console.log(stdout);
            if (err) {
                console.error(`exec error: ${err}`);
                return;
